...
### 3. ⚠️ CRITICAL: Filter Prop Requires `search: true`

**If you use `:filter` prop on the client to pre-filter data, the field MUST have `search: true` on server.**

```vue
<!-- Client: Filter PRDs by project -->
<h-crud entity="prd" :filter="{ project: currentProject._id }" ... />
```

```javascript
// Server: MUST have search: true for filter to work
{ name: "project", type: "string", ref: "project", search: true }  // ✅ Filter works
{ name: "project", type: "string", ref: "project", search: false } // ❌ Filter IGNORED!
```

**Common mistake:** Setting `search: false` to hide a field from search form, then wondering why `:filter` doesn't work.

**Solution:** Keep `search: true` on server, use `hideSearch` on client to hide from UI:
...
<h-crud entity="product" :search-fields="['name', 'sku', 'status']">
...
- **hola-crud-hide-search** - Hide fields from search form while keeping server filtering (use with `:filter` prop)
