...
- `search`, `list` → **default to false** (can be explicitly set to true to opt-in)
- Client cannot set values, but hooks CAN set them server-side

```typescript
{ name: "created_by", sys: true },  // Auto: create/update/clone = false, search/list = false
{ name: "created_at", sys: true, list: true },  // Visible in list despite being sys
```

### `secure: true` - Secure Fields

Fields that should not be exposed in lists or search (e.g., passwords):

- `list`, `search` → **must be false** (throws error if set to true)
- Excluded from `property_fields` and `list_fields`

```typescript
{ name: "password", secure: true },  // Auto: list/search = false
```
...
- `search`, `list` → **default to false** (can be explicitly set to true)
...
| Subset            | Filter Logic                                              |
| ----------------- | --------------------------------------------------------- |
| `client_fields`   | `sys !== true \|\| list === true \|\| search === true`    |
| `property_fields` | `list !== false && secure !== true`                       |
...
> [!NOTE] `client_fields` includes sys fields that explicitly opt-in with `list: true` or `search: true`.
...
The `role` attribute on fields controls which user roles can see the field:

```typescript
fields: [
  { name: "public_info" },                    // Visible to all roles
  { name: "internal_notes", role: "admin" },   // Admin only
  { name: "audit_log", role: ["admin", "auditor"] },  // Admin or auditor
  { name: "shared_field", role: "*" },         // Explicitly all roles
]
```

Filter logic: fields without `role` are visible to all. Fields with `role` are filtered by `filter_fields_by_role()` based on the user's role.
