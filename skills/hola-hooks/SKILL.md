...
```typescript
{
  list_query: async ({ entity, query }) => {
    // Add default filter to all list queries
    query.published = true; // Only show published items

    // Modify date range
    if (query.created_at) {
      query.created_at = {
        $gte: new Date(query.created_at as string),
...
> [!IMPORTANT] `list_query` context only has `{ entity, query }`. It does NOT have access to user info (`_user`). For user-based filtering, use `user_field` in meta definition instead.
